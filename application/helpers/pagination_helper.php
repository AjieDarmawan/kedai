...
if( !function_exists('gen_paging')) {
	function gen_paging($page_data = array()) {
		...
		/*echo "row count ".$page_data['count_row']."\n";
		echo "countny ".$count;
		echo "limit ".$page_data['limit']."\n";
		echo "page current ".$page_data['current'];*/
		
		$page_result .= '<ul class="pagination pagination-sm no-margin pull-right">
							<li ' . ( $page_data['current'] == 1 ? 'class=""' : '' ) . '><a href="javascript:void(0)" ' . ($page_data['current'] == 1 ? '' : 'onclick = "'.$func_name.'(1)"') .' >&laquo; First</a></li>';
		...
		$page_result .= '<li ' . ( $page_data['current'] == $page_end ? 'class=""' : '' ) . '><a href="javascript:void(0)" onclick = "'.$func_name.'('.$count.')">Last &raquo;</a></li>';
		$page_result .='</ul>';
